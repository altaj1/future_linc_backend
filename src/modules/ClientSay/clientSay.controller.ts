import { Request, Response } from "express";
import { BaseController } from "@/core/BaseController";
import { ClientSayService } from "./clientSay.service";
import { HTTPStatusCode } from "@/types/HTTPStatusCode";

export class ClientSayController extends BaseController {
  constructor(private service: ClientSayService) {
    super();
  }

  /**
   * Create a new ClientSay
   */
  public create = async (req: Request, res: Response) => {
    const body = req.validatedBody;
    this.logAction("create", req, { body });
    const userId = req.userId;
    const result = await this.service.create(body, userId, {});

    return this.sendCreatedResponse(
      res,
      result,
      "ClientSay created successfully",
    );
  };

  /**
   * Get all ClientSays
   */
  public getAll = async (req: Request, res: Response) => {
    const pagination = this.extractPaginationParams(req);
    this.logAction("getAll", req, { pagination });

    const result = await this.service.findMany({}, pagination);

    return this.sendPaginatedResponse(
      res,
      {
        page: result.page,
        limit: result.limit,
        total: result.total,
        totalPages: result.totalPages,
        hasNext: result.hasNext,
        hasPrevious: result.hasPrevious,
      },
      "ClientSays retrieved successfully",
      result.data,
    );
  };

  /**
   * Get single ClientSay
   */
  public getOne = async (req: Request, res: Response) => {
    const { id } = req.validatedParams;
    this.logAction("getOne", req, { id });

    const result = await this.service.findById(id);

    if (!result) {
      return this.sendResponse(
        res,
        "ClientSay not found",
        HTTPStatusCode.NOT_FOUND,
      );
    }

    return this.sendResponse(
      res,
      "ClientSay retrieved successfully",
      HTTPStatusCode.OK,
      result,
    );
  };

  /**
   * Update ClientSay
   */
  public update = async (req: Request, res: Response) => {
    const { id } = req.validatedParams;
    const body = req.validatedBody;
    this.logAction("update", req, { id, body });

    const exists = await this.service.exists({ id });
    if (!exists) {
      return this.sendResponse(
        res,
        "ClientSay not found",
        HTTPStatusCode.NOT_FOUND,
      );
    }

    const result = await this.service.updateById(id, body);

    return this.sendResponse(
      res,
      "ClientSay updated successfully",
      HTTPStatusCode.OK,
      result,
    );
  };

  /**
   * Delete ClientSay
   */
  public delete = async (req: Request, res: Response) => {
    const { id } = req.validatedParams;
    this.logAction("delete", req, { id });

    const exists = await this.service.exists({ id });
    if (!exists) {
      return this.sendResponse(
        res,
        "ClientSay not found",
        HTTPStatusCode.NOT_FOUND,
      );
    }

    await this.service.deleteById(id);

    return this.sendResponse(
      res,
      "ClientSay deleted successfully",
      HTTPStatusCode.OK,
    );
  };
}
