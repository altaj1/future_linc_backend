...
export class AuthService extends BaseService<User> {
     ...
     * Update profile
     */
    async updateProfile(userId: string, data: UpdateProfileInput, imageFile?: Express.Multer.File): Promise<Omit<User, 'password'>> {
        const user = await this.findById(userId);
        if (!user) {
            throw new NotFoundError('User not found');
        }
        ...
        if (imageFile) {
            if (user.imagePublicId) {
                await deleteLocalFile(user.imagePublicId, "avatars");
            }
            const uploaded = await uploadToLocal(`user-${userId}`, imageFile.path, "avatars");
            imageData = uploaded;
        }
