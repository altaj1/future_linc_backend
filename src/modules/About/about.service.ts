...
> {
  ...
  public async create(
    ...
    if (files?.image1?.[0]) {
      image1Data = await uploadToLocal(
        `about-${data.section}-1`,
        files.image1[0].path,
        "about",
      );
    }
    if (files?.image2?.[0]) {
      image2Data = await uploadToLocal(
        `about-${data.section}-2`,
        files.image2[0].path,
        "about",
      );
    }
  ...
  public async updateById(
    ...
    if (files?.image1?.[0]) {
      ...
      image1Data = await uploadToLocal(
        `about-${data.section || existingRecord.section}-1`,
        files.image1[0].path,
        "about",
      );
    }
    ...
    if (files?.image2?.[0]) {
      ...
      image2Data = await uploadToLocal(
        `about-${data.section || existingRecord.section}-2`,
        files.image2[0].path,
        "about",
      );
    }
